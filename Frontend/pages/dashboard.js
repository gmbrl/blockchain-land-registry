...
const dashboard = () => {
  ...
  async function SendOTP(Mwaura) {
    ...
    async function VerifyOTP(user,Mwaura, otp) {
      ...
      try{
        ...
        alert(response.data);
        console.log(
          "User "+Mwaura+" Sucessfull Verified!!"
        )        
        if (user == 'seller'){
  ...
  const onFinishseller = (values) => {
    VerifyOTP(
      'seller',
      values['Mwaura Number'],
      values['otp']
    )
  };
  const onFinishFailedseller = (errorInfo) => {
    if (errorInfo.values['Mwaura Number']){
      console.log("Failed:", errorInfo.values['otp']);
      SendOTP(errorInfo.values['Mwaura Number']);
    }
    else{
    alert(
      "Please inser Correct Mwaura Card"
    )
    }
  ...
  return (
    <div className="bg-gray-200 max-h-screen overflow-y-auto overflow-x-hidden" >
      ...
      <Row gutter={16} className="justify-center py-64">
        ...
        >
          <Card title="SELLER" bordered={false}>
        ...
        >
          ...
          <ul className="font-bold text-lg mb-2 text-center">FUNCTIONS OF Client</ul>
            <ul>
              <li className="text-center">CONNECT TO METAMASK</li>
        ...
        >
          ...
          <ul className="font-bold text-lg mb-2 text-center">MINSISTRY OF LAND</ul>
            <ul>
